...
namespace Network01_Shared
{
    ...
    public class ServerOTPC<T> where T : ServerOTPC<T>.ConnectionHandler, new() //utilizzare i generics risparmia all'utente della mia classe pericolosi downcast
    {
        ...
        public abstract class ConnectionHandler : IDisposable
        {
            ...
            protected abstract void OnDataReceived(byte bId, ushort uSize, BinaryReader hReader);
            ...
            private void ConnectionThread()
            {
                try
                {
                    while (true)
                    {
                        byte bId        = m_hReader.ReadByte();
                        ushort uSize    = m_hReader.ReadUInt16();
                        if (uSize > Socket.ReceiveBufferSize - 3)
                            throw new Exception("Buffer Limit Exceded on client");
                        
                        OnDataReceived(bId, uSize, m_hReader);
                    }
                }
                catch (Exception)
                {
